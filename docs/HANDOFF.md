...
| 슬랙 알림 | 봇 `@alimi`(`SLACK_BOT_TOKEN` 1개) + 채널 상수 `src/constants/slack-channels.ts`. ① 평일 16:00 당일 채널 기록을 출석/진찰 DB에 반영한 뒤 미출석/미진찰 명단(실명) + 월간 리포트 요약 → `#마루-진찰`(채널 ID `C0B9LCED676`) ② 매일 08:30 회원("환자"의 원내 호칭) 생일 알림 → `#마루` (마루 = 대동병원 낮병원 이름). 상세: `docs/superpowers/plans/2026-06-10-slack-noon-report.md`. 진찰 알림 엔드포인트는 방어적으로 16:00 KST 전 호출을 `before_report_window`로 skip한다. 정기 알림은 앱 직송 — 헤르메스 에이전트 경유 금지(장애 시 누락). CRON_SECRET은 2026-06-11 rotate (Vercel env·.env.local·cron.job 세 곳 동일값). **새 알림 추가는 `docs/cron-jobs-guide.md` 참고** (compose 순수함수 + cron 라우트 복사 + 채널 상수 + pg_cron 잡 1줄, 환경변수 불필요. 배포→pg_cron 등록 순서 주의) |
