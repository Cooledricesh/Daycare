...
정신과 **낮병원 환자 출석/진찰 관리 시스템**. 실서비스 운영 중 (실제 환자 데이터 362명).
...
- SQL 파일은 `supabase/migrations/`에 계속 작성하고, NAS에서 호환되지 않는 Storage/transaction 차이는 `nas/migrations/` variant로 둔다.
...
1. **Daycare PostgREST는 서버측 1000행 캡** — 대량 조회는 반드시 `fetchAllPaginated` (`src/lib/supabase-pagination.ts`). 이 함수는 5페이지 wave 병렬이며, 안정적 페이지 경계를 위해 쿼리에 unique 컬럼 order 필수
