...
> Vercel Cron은 UTC 기준. `00:30 KST = 15:30 UTC (전일)` → `30 15 1 * *`는 매월 1일 UTC 15:30 = KST 2일 00:30. 의도한 "1일 00:30 KST 생성"을 위해서는 `30 15 말일 * *`이어야 하지만 cron 표현식으로 "말일"을 지정 불가. 따라서 **"매월 1일 KST 00:30 → 전월 리포트 생성"** 의미로 해석:
>
> - `30 15 * * *` 매일 UTC 15:30 실행 + 함수 내부에서 "1일이면 전월 리포트 생성, 아니면 종료"
> - 또는 간단히 `30 15 1 * *` (UTC 1일 15:30 = KST 2일 00:30)로 하고, 리포트 대상은 **2일 기준 전월** (즉 어제의 월). 실용상 동일 효과.

**채택안**: `30 15 1 * *` + 서버는 "KST 기준 어제가 속한 달의 리포트" 생성.
